...
@Service
class GetPartnerInsurerSummaryProjectionQueryHandler(
    private val repository: PartnerInsurerQueryRepository,
) : QueryHandler<GetPartnerInsurerQuery, PartnerInsurerProjection.SummaryProjection?> {
    ...
    override suspend fun invoke(
        ...
        return when (query.identifier) {
            is GetPartnerInsurerQuery.Identifier.ById -> {
                repository.findByIdSummary(query.identifier.id) ?: throw EntityNotFoundException(
            ...
            is GetPartnerInsurerQuery.Identifier.ByPartnerCode -> {
                repository.findByPartnerCodeSummary(query.identifier.partnerInsurerCode)
            ...
            is GetPartnerInsurerQuery.Identifier.ByTaxIdentificationNumber -> {
                repository.findByTaxIdentificationNumberSummary(query.identifier.taxIdentificationNumber)
                    ?: throw EntityNotFoundException(
                        getAggregateTypeOrEmpty<PartnerInsurer>(),
